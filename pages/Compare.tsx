...
const Compare: React.FC = () => {
  ...
  const fundsByType = useMemo(() => {
    const groups: Record<string, typeof MOCK_FUNDS> = {
        'ETF': [],
        'Equity': [],
        'Balanced': [],
        'Bond': []
  ...
  const riskRewardData = useMemo(() => {
     return selectedChartFundIds.map(id => {
     ...
     }).filter(Boolean) as { id: string, name: string, fullName: string, return: number, volatility: number, expenseRatio: number }[];
  }, [fundHistories, selectedChartFundIds]);
  ...
  // 6. Bar Chart Data (Annual Performance) - Static for simplicity or reuse MOCK directly
  const annualBarDataSimple = useMemo(() => {
     const fundsToChart = MOCK_FUNDS.filter(f => selectedChartFundIds.includes(f.id));
     if (fundsToChart.length === 0) return [];
