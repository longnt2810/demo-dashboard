...
const CompoundInterestCalculator: React.FC = () => {
  ...
  const CustomTooltip = ({ active, payload, label }: any) => {
    if (active && payload && payload.length) {
      ...
      return (
        <div className="bg-white dark:bg-slate-900 p-4 rounded-xl shadow-2xl border border-slate-100 dark:border-slate-800 min-w-[260px]">
          {/* Header */}
          <div className="mb-3 flex justify-between items-center border-b border-slate-100 dark:border-slate-800 pb-2">
          ...
          {/* Hero Section: Value & Growth */}
          <div className="mb-5">
             <div className="text-3xl font-extrabold text-slate-900 dark:text-white tracking-tight leading-none mb-2">
               {formatVND(total)}
             </div>
             
             {/* Growth Indicator - Moved Up */}
             <div className={`flex items-center gap-2 ${pointGrowth > 0 ? 'opacity-100' : 'opacity-0'}`}>
                <div className="flex items-center gap-1 bg-emerald-100 dark:bg-emerald-500/20 px-2 py-0.5 rounded-md">
                    <TrendingUp className="h-3.5 w-3.5 text-emerald-600 dark:text-emerald-400" />
                    <span className="text-sm font-bold text-emerald-700 dark:text-emerald-400">
                        +{pointGrowth.toFixed(1)}%
                    </span>
                </div>
                <span className="text-xs text-slate-400 font-medium">tăng trưởng</span>
             </div>
          </div>
          ...
          <div className="space-y-2 bg-slate-50 dark:bg-slate-800/50 p-3 rounded-lg">
            ...
            <div className="flex justify-between items-center text-sm">
               <div className="flex items-center gap-2">
                  ...
                  <span className="text-slate-600 dark:text-slate-300 font-medium text-xs">Vốn gốc</span>
               </div>
               <span className="font-bold text-slate-700 dark:text-slate-200 text-xs tabular-nums">{formatVND(principal)}</span>
            </div>
            ...
            <div className="flex justify-between items-center text-sm">
               <div className="flex items-center gap-2">
                  ...
                  <span className="text-slate-600 dark:text-slate-300 font-medium text-xs">Lợi nhuận</span>
               </div>
